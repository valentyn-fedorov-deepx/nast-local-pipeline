...
class PolarFrame:
    ...
    @staticmethod
    def rot_disp(n):
        """Sensor-frame in-plane components -> DISPLAY frame (image is shown
        rotated 90 deg CW / k=3): display right = -sensor y, display down =
        +sensor x  =>  nx' = -ny, ny' = nx (nz unchanged)."""
        return np.stack([-n[..., 1], n[..., 0], n[..., 2]], -1)

    def aolp_disp(self):
        """controller theta against the DISPLAY x-axis (+90 deg for the roll,
        wrapped back to (-pi/2, pi/2])."""
        ph = np.mod(self.theta_display() + np.pi / 2, np.pi)
...
# ------------------------------------------------------------------ products table
def products(frame, n_refr=1.5, k_att=0.0, gamma_off=0.0):
    ...
    out = {
        ...
        "nxyz_phys":      f.encode(f.rot_disp(phys), "canon", signed=True),
        "nxyz_diffuse":   f.encode(f.rot_disp(f.px_diffuse(n_refr, gamma_off)), "canon", signed=True),
        "nxyz_specv2":    f.encode(f.rot_disp(f.px_specular_v2(n_refr, k_att, gamma_off)), "canon", signed=True),
        "edge":           f.edge(ps),
    }
    return out
